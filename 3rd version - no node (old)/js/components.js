...
Vue.component('dx-header', {
    ...
    <header>
            <div id="navbar" :style="{ 'background': 'url(' + navbar_background + ')' }">
                <div class="logo" :style="{ 'background': 'url(' + logo_background + ')' }">
                ...
                <div class="search_field">
                    <form action="POST">
                        <input type="text" :placeholder="$t('placeholder.search')" name="search_input" :style="{ 'background': 'url(' + search_field + ')' }">
                    </form>
                </div>
                <div class="expanded">
                    <a href="./submit_dapp.html">
                        <div class="submit_btn">{{$t("button.submit_dapp")}}</div>
                    </a>
                    <div class="language" :alt="$t('alt.select_language')" :title="$t('alt.select_language')" @click="$i18n.locale==='en'?$i18n.locale='ru':$i18n.locale='en'">
                        <div class="lang_icon">
                            <img :src="language">
                        </div>
        ...
        <div class="main_content">
            <div class="social">
                ...
                <div class="social_buttons">
                    <a v-for="(item, key) in social" :href="item.href" :alt="key" :title="key">
            ...
            <div class="other_links">
                <div class="link_row">
                    <a class="link" href="./">{{$t('footer_link.submit_dapp')}}</a>
                    <a class="link" href="./">{{$t('footer_link.terms_of_use')}}</a>
                    <a class="link" href="./">{{$t('footer_link.cookie_policy')}}</a>
                </div>
                <div class="link_row">
                    <a class="link" href="./">{{$t('footer_link.coin_gecko_api')}}</a>
                    <a class="link" href="./">{{$t('footer_link.privacy_policy')}}</a>
                </div>
            </div>
        </div>
        <div class="copyright">{{ $t('message.copyright') }} © {{ new Date().getFullYear() }} Dapp.expert</div>
        <img class="footer_background_reflection" :src="footer_background_reflection" alt="">
    </footer>
    `,
});
// !Master
...
// Index
Vue.component('top-ads', {
    data() {
        return {
            top_ads: top_ads, //mock data
        }
    },
    template: `<section id="top_ads">
                <div class="big_ad">
                    <a href="#">
                        <div class="img_container">
                            <img :src="top_ads.big_ad.img" :alt="top_ads.big_ad.alt" :title="top_ads.big_ad.alt">
                        </div>
                    </a>
                </div>
                <div class="small_ads">
                    <a href="#" v-for="ad in top_ads.small_ad">
                        <div class="img_container">
                            <img :src="ad.img" :alt="ad.alt" :title="ad.alt">
                        </div>
                    </a>
                </div>
            </section>`,
...
Vue.component('blockchains-bar', {
    data: function() {
        return {
            ...
            blockchains: blockchains, // mock data
        };
    },
    template: `<section id="blockchain">
                <div class="blockchains_container" :style="{ 'background': 'url(' + blockchain_background + ')' }">
                    <div class="blockchain_label">{{$t('message.blockchains')}}</div>
                    <div class="blockchain_buttons">
                        <a href="">
                            <div class="blockchain_btn active" :style="{ 'background': 'url(' + trapezio_blue_btn + ')' }">{{$t('button.all')}}</div>
                        </a>
                        <a v-for="blockchain in blockchains" href="">
                            <div class="blockchain_btn" :style="{ 'background': 'url(' + trapezio_grey_btn + ')' }">{{blockchain.acronym}}</div>
                        </a>
                    </div>
                </div>
            </section>`,
...
Vue.component('dapp-table', {
    data: function() {
        return {
            trapezio_blue_btn: "img/trapezio_blue_btn.png",
            trapezio_grey_btn: "img/trapezio_grey_btn.png",
            sort_icon: "img/sort_icon.svg",
            dapp_list: dapp_list, // mock data
    ...
    template: `<section id="dapp_table">
                <table cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                            <th>
                                <div class="period_select">
                                    <a href="">
                                        <div id="_24h" class="period active" :style="{ 'background': 'url(' + trapezio_blue_btn + ')' }">{{$t('button._24h')}}</div>
                                    </a>
                                    <a href="">
                                        <div id="_7d" class="period" :style="{ 'background': 'url(' + trapezio_grey_btn + ')' }">{{$t('button._7d')}}</div>
                                    </a>
                                </div>
                            </th>
                            <th>
                                ...
                                <a href="">
                                    <div class="thead_item">
                                        <div class="breaklines">
                                            ...
                                            <br> {{$t('message.dapp_expert_score._1')}}
                                            <br>{{$t('message.dapp_expert_score._2')}}
                                        </div>
                                    </div>
                                </a>
                            </th>
                    ...
                    <tbody>
                        <tr v-for="dapp in dapp_list" @mouseover="dapp.hover = true" @mouseleave="dapp.hover = false">
                            <td>
                                ...
                                <div class="cell">
                                    ...
                                    <div class="percent_bar_container">
                                        <div class="percent_bar">
                                            <img v-if="dapp.hover" class="percent_bar_background" :src="percent_bar_background_active">
                ...
                <div class="see_more">
                    <div class="text">{{$t('message.see_more')}}</div>
                </div>
            </section>`,
});
...
Vue.component('info-section', {
    data: function() {
        return {
            info_item: info_item, //mock data
        };
    },
    template: `<section id="info">
                <div v-for="(item, key, index) in $t('info_item')" :class="{'border' : index === Object.keys($t('info_item')).length-1, 'info_item' : 1}">
                    <div class="info_title">{{ item._0 }}<br>{{item._1}}</div>
                    <div class="info_value">{{ info_item[key] }}</div>
                </div>
            </section>`,
});

Vue.component('graphic-section', {
    data: function() {
        return {
            big_graphic: "img/big_graphic.png",
        };
    },
    template: `<section id="graphic_section">
                <div class="upper_row">
                    <div class="title">{{$t('message.blockchains_data_info')}}</div>
                    <div class="buttons">
                        <div v-for="(button, _, index) in $t('button.graphic_info')" :class="{'button_selector' : 1, 'active' : index === 0}">{{button}}</div>
                    </div>
                </div>
                <div class="graphic">
                    <img :src="big_graphic">
